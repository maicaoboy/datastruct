 ...
 * @param D  存储从v0到其余各顶点的最短路径代价，在图中，该代价是步长和，在网中，该代价是权值和。
 */
void ShortestPath_DIJ_Net(MGraph G, int v0, PathMatrix_DIJ P, ShortPathTable_DIJ D) {
    int v, w, i, j, min;
    Status final[MAX_VERTEX_NUM];   // 用来标记当前结点是否已经访问过，即是否已加入最短路径
 ...
 * @param D  存储从v0到其余各顶点的最短路径代价，在图中，该代价是步长和，在网中，该代价是权值和。
 */
void ShortestPath_DIJ_Net_PathMatrix(MGraph G, int v0, PathMatrix P,ShortPathTable D) {
    int i, j, min, v;
    ...
    for(i = 0; i < G.vexnum; i++) {
        ...
        for(j = 0; j < G.vexnum; j++) {
            if(!final[j]) {
                if(min != INFINITE && G.arcs[v][j].adj != INFINITE) {
                    if(G.arcs[v][j].adj + min < D[j]) {

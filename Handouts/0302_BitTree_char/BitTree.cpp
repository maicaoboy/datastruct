...
BitNode* createBitTreeLevelMiddle(ElemType levelOrderArray[], int levelBegin, int levelEnd,
    ...
    int index = midBegin;
    while(index < midEnd && middleOrderArray[index] != levelOrderArray[levelBegin]) {
        index++;
    }

    ElemType leftLevelOrderArray[MAX_SIZE] = {0};
    int leftSize = 0;
    ElemType rightLevelOrderArray[MAX_SIZE] = {0};
    int rightSize = 0;

    for(int i = levelBegin + 1; i <= levelEnd; i++) {
        for(int j = midBegin; j < index; j++) {
...
/**
 * 例题13:一个仅包含二元运算符的算术表达式一二叉链表的形式存放在二叉树 T 中,设计算法求解算术表达式值.
 * @param T
 * @return
 */
double calculateExpress(BitTree T) {
    if(T == NULL) {
        return 0L;
    }
    if(T->lchild == NULL || T->rchild == NULL) {
    ...
    double left = calculateExpress(T->lchild);
    double right = calculateExpress(T->rchild);
    double result = 0;
    switch (T->data) {
        case '+':
            result = left + right;

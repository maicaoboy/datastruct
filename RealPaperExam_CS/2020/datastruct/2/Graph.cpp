...
void printfPath(QElemType1 queue[MAX_VERTEX_NUM], int index){
    VertexType path[MAX_VERTEX_NUM];
    int length = 0;
    printf("从 %d 到 %d 的最短路径为: ", queue[1].v, queue[index].v);
    while(index != -1) {
        path[length++] = queue[index].v;
        index = queue[index].index;
    }
...
/**
 * 2020CS-五-2
 * 设不带权有向无环图 G，以邻接表方式存储。编写算法，求图G中顶点S到顶点d的最知路径
 * 算法思想:从s顶点出发使用广度优先遍历找到d顶点,找到d顶点后即可通过递归查找节点来源顶点找到最短路径
 */
void BFS_ShortestPath(ALGraph G, VertexType s, VertexType d) {
    QElemType1 queue[MAX_VERTEX_NUM];
    int qLength = 1;
    int cur = 1;
    int visited[MAX_VERTEX_NUM] = {0};

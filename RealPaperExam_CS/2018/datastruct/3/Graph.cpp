...
/**
 * 编写算法，判断一个以邻接表存储结构的无向图G是否连通，若连通，返回1，否则，返回0。
 * 算法思想: 从任意一个结点出发进行DFS遍历,完成一次遍历后若visited数组还存在未被访问过的顶点,则不连通
 * @param G
 * @return
 */
...
void DFS_2019CS(ALGraph G, VertexType v) {
    visited1[v] = 1;
    ArcNode *nextArc = G.vertices[v].firstarc;
    while(nextArc != NULL) {
        if(!visited1[nextArc->adjvex]) {
            DFS_2019CS(G, nextArc->adjvex);
        }
        nextArc = nextArc->nextarc;
    }
}

int isLiantong(ALGraph G) {
    VertexType v = 1;
    DFS_2019CS(G, v);
    for (int i = 1; i <= G.vexnum; ++i) {
        if(visited1[i] == 0) {
...
int isExistPathK_DFS(ALGraph G, VertexType v, VertexType w, int k) {
    if(k == 0 && v == w) {
        return true;
    }else if(k == 0) {
        visited2[v] = 0;
    ...
    ArcNode *nextArc = G.vertices[v].firstarc;
    while(nextArc != NULL) {
        if(!visited2[nextArc->adjvex]) {
            if(isExistPathK_DFS(G, nextArc->adjvex, w, k - 1)){
                return 1;
            }
        }
        nextArc = nextArc->nextarc;

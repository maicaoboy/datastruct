...
void getWidth(BSTree T, int level) {
    if(T == NULL) return;
    if(level > maxDepth) {
        maxDepth = level;
    }
...
int cnt = 0;
void preOrder_1(BSTree T, int level, int k) {
    if(T == NULL) return;
...
BSTNode *nodeList[MAX_SIZE] = {NULL};
 ...
 * @param level             当前层数
 */
void findfarestLeaf_Pre(BSTree T, BSTNode *parent, int level) {
    if(T == NULL) return;
    T->parent = parent;
    T->level = level;
    if((T->lchild == NULL) && (T->rchild == NULL)) {
        if(nodeListLength == 0 || nodeList[nodeListLength - 1]->level == level){
 ...
 */
void printPath(BSTNode *nodeList[MAX_SIZE], int nodeListLength){
